...
pidgeotto_win = 0
mewtwo_win = 0
global_vars_file = "pokemon_wins.txt"

def read_global_vars():
    global bulbasaur_win, charmander_win, squirtle_win, eevee_win, pikachu_win, seel_win, ponyta_win, poliwag_win, pidgeotto_win, mewtwo_win


    with open(global_vars_file, "r") as file:
        lines = file.readlines()
        bulbasaur_win = int(lines[0].strip())
        charmander_win = int(lines[1].strip())
        squirtle_win = int(lines[2].strip())
        eevee_win = int(lines[3].strip())
        pikachu_win = int(lines[4].strip())
        seel_win = int(lines[5].strip())
        ponyta_win = int(lines[6].strip())
        poliwag_win = int(lines[7].strip())
        pidgeotto_win = int(lines[8].strip())
        mewtwo_win = int(lines[9].strip())
        
        
def write_global_vars():
    with open(global_vars_file, "w") as file:
        file.write(f"{bulbasaur_win}\n")
        file.write(f"{charmander_win}\n")
        file.write(f"{squirtle_win}\n")
        file.write(f"{eevee_win}\n")
        file.write(f"{pikachu_win}\n")
        file.write(f"{seel_win}\n")
        file.write(f"{ponyta_win}\n")
        file.write(f"{poliwag_win}\n")
        file.write(f"{pidgeotto_win}\n")
        file.write(f"{mewtwo_win}\n")
...
def batalha():
        ...
        def get_win(self):
            ...
            if self.current_hp > 0:
                if self.name == "Bulbasaur":
                    ...
                    seel_win += 1
                elif self.name == "Ponyta":
                    ponyta_win += 1
                elif self.name == "Poliwag":
                    poliwag_win += 1
                elif self.name == "Pidgeotto":
                    pidgeotto_win += 1
    ...
    def create_button(width, height, left, top, text_cx, text_cy, label):
        ...
        return button
    def play_music(file_path):
        musica = pygame.mixer.music.load(file_path)
        pygame.mixer.music.play(-1)
    ...
    #cria a variaveis iniciais de vitória
    ...
    while game_status != "quit":
        ...
        if game_status == "gameover":
            print(f'Bulbasaur: {bulbasaur_win} vitórias')
            print(f'Charmander: {charmander_win} vitórias')
            print(f'Squirtle: {squirtle_win} vitórias')
            print(f'Eevee: {eevee_win} vitórias')
            print(f'Pikachu: {pikachu_win} vitórias')
            print(f'Seel: {seel_win} vitórias')
            print(f'Ponyta: {ponyta_win} vitórias')
            print(f'Poliwag: {poliwag_win} vitórias')
            print(f'Pidgeotto: {pidgeotto_win} vitórias')
            print(f'Mewtwo: {mewtwo_win} vitórias')
